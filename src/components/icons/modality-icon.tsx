...
import { Bone, Wifi, Magnet } from "lucide-react";

interface ModalityIconProps extends React.SVGProps<SVGSVGElement> {
    modality?: string;
}

export const ModalityIcon = ({ modality, ...props }: ModalityIconProps) => {
    const mod = modality?.toUpperCase();

    if (mod === 'RX') return <Bone {...(props as any)} />;
    if (mod === 'ECO') return <Wifi {...(props as any)} />;
    if (mod === 'RMN') return <Magnet {...(props as any)} />;
    
    // Default / TAC icon (Radiation source)
    return (
